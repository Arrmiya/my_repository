...
try{
  let d=Object.getOwnPropertyDescriptor(Location.prototype,'href');
  if(d&&d.set){let _s=d.set;Object.defineProperty(Location.prototype,'href',{get:d.get,set:function(v){if(blocked(v)){console.log('[Blk href]',v.split(':')[0]);return;}_s.call(this,v);},configurable:true});}
}catch(e){}
try{let _a=Location.prototype.assign;Location.prototype.assign=function(v){if(blocked(v)){console.log('[Blk assign]',v.split(':')[0]);return;}_a.call(this,v);};}catch(e){}
try{let _r=Location.prototype.replace;Location.prototype.replace=function(v){if(blocked(v)){console.log('[Blk replace]',v.split(':')[0]);return;}_r.call(this,v);};}catch(e){}
...
function onReady(){
  ...
  sb.id='__sb';sb.style.cssText='position:fixed;top:0;left:0;right:0;height:36px;z-index:2147483646;pointer-events:none;';
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(move |app| {
            // 关闭默认创建的 WebView，用带 init_script 的新建替换
            ...
            println!("[Browser] 应用启动完成 (含 init_script)");
            Ok(())
        })
